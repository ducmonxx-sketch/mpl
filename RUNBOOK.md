...
- **5c manual status-edit guard:** driver/vehicle PATCH to ACTIVE/AVAILABLE now 409s while an OCCUPYING shipment still references them (only the dangerous direction — UNAVAILABLE/MAINTENANCE stay unconditionally allowed, the substitute-driver escape hatch). Cosmetic frontend: DriversSection/ArmadaSection edit modals disable "Tersedia" for a currently-engaged driver/vehicle (backend is still the real gate).
- **5d TrackingSection (SHARED — gated behind isAdmin where UI-visible):** 8s poll no longer re-opens a closed detail panel (`autoSelectedRef`, resets on `initialSearchQuery` change); status-change no longer unconditionally sends `proofPhoto: null`; adding a checkpoint now refetches the timeline immediately (was invisible until deselect/reselect); backend `PATCH /tracking/events/:eventId` now 404s on an unknown id (was a generic 500) and writes an audit log (new `UPDATE_SHIPMENT_EVENT` action type + migration `add_update_shipment_event_action_type`, mirroring `UPDATE_ADMIN` from package 4).
- **⚠️ Discovered while verifying 5d, not fixed:** `proofPhoto`/`estimatedArrival` don't exist on `Shipment` — dropped by migration `drop_shipment_price_eta` back on **2026-07-15**. The `/status` route only ever reads `status`/`currentProgressPercent`. So TrackingSection's ETA-picker and proof-photo-upload Admin Controls have been silently no-ops for ~2 months (the plan's "wipe" framing for 5d#2 assumed the column still existed — it verified as harmless hygiene, not an active bug). Needs a product decision (restore columns vs strip UI) — flagged in `client-deployment.md` and DEV-PLAN, not actioned.
- **Verified:** typecheck 0 · smoke **66/66** (+7 new probes: dup-plate 409, delete-guard ×2, status-edit-guard ×2, bogus-event-id 404) · `vite build` green.
- **Cut short by a token-budget stop** mid-session (user: "token is about to be filled") — code + tests are done and committed; the three affected `docs/pages/*.md` (admin-drivers, admin-armada, tracking-shared) were **not** updated this pass — flagged as the immediate next TODO in DEV-PLAN. Package 8 not started.
...
- **4b PIC edit + company rename** — `ClientsSection.jsx`: edit modal now has a "Nama PIC" field (`editFullName`, seeded from the real PIC name) instead of hardcoding `fullName: 'Admin Perusahaan'` on every save; a company-name change is detected (`editOriginalCompanyName`) and routed through a **new bulk endpoint** `PATCH /api/users/company-rename` (`clientManagerOnly`, registered before `/:id`) instead of splitting the company via a per-user PATCH.
- **4c SUPPORT read-only** — confirmed via `lib/rbac.ts` (`SUPPORT: []`) and `statusFlow.ts` that SUPPORT has no legal forward move. Deleted the dead "Fallback for OPERATIONS/SUPPORT" branches in both `handleConfirmStatus` and `renderModalContent` (PENDING-assign, DITUGASKAN→TRANSIT which always 403'd, generic TRANSIT picker) plus the now-orphaned `newDriverVehicleId` state; `canUpdateStatus` explicitly returns `false` for SUPPORT. Caught by lint after the fact: a leftover `setNewDriverVehicleId('')` in `resetModalState` (no-undef) — fixed.
- **4d Admin Profil de-faked** — was mock `setTimeout` activity + a simulated save toast. New `GET /api/audit-logs/me` (self-scoped, any role — `adminId` forced server-side, sibling to the SUPERADMIN-only `GET /api/audit-logs`) and `PATCH /api/auth/admin/me` (fullName only — **Admin has no `phoneNumber` column**, checked schema first). Needed a **new migration** `add_update_admin_action_type` (`UPDATE_ADMIN` added to `AuditActionType` enum — the plan's assumed value didn't exist). Email field disabled in the UI (was editable-looking but not sendable — same fake pattern as the activity log).
- **4e `/assign` fleet mirror** — was leaving driver/vehicle at ACTIVE/AVAILABLE after PENDING→DITUGASKAN. Now releases the outgoing pair if changed (group-aware via `releaseFleetIfUnused`) and engages the new pair via `mirrorFleetStatus`, matching `/status`/`/plant-check`/`/handover`.
- **Verified:** typecheck 0 · smoke **58/58** (+8 new probes: no-legacy-token, CSRF-403, `/assign` mirror check, self-update + self-log round trip) · `vite build` green · `npm run lint` shows **zero new** issues (56 pre-existing errors untouched, confirmed via isolated per-file lint runs and a diff review).
- **Docs updated in the same pass** (CLAUDE.md rule): `admin-shipments.md`, `admin-clients.md`, `admin-profile.md`; `client-deployment.md` package 4 ticked.
- **Not pushed** — commits local, per plan ("Nothing pushed without explicit user OK").
