...
**File:** `apps/web/src/pages/AdminComponents/ShipmentsSection.jsx` · **Roles:** all six admin roles. SUPERADMIN + OPERATIONS (`STATUS_OVERRIDE_ROLES`) get the tabbed table, free status picker, direct-assign row button, and delete. KEPALA_ARMADA / PIC_PABRIK / PIC_GUDANG (`usesFieldLayout`) get a compact field layout: status dropdown instead of tabs, centered detail modal, Dalam Proses/Selesai toggle, and one scoped status step each. **SUPPORT is read-only on shipment status** (`[]` permissions in `lib/rbac.ts` — no legal forward move; `canUpdateStatus` returns `false`, no fallback flow — fixed 2026-09-22, was previously a broken DITUGASKAN→TRANSIT attempt that always 403'd). KEPALA_ARMADA + OPERATIONS (`SHIPMENT_CREATOR_ROLES`) can create shipments.
...
| `shipmentsAPI.assign(id, data)` | `PATCH /api/shipments/:id/assign` | Set driver/vehicle (+optional pickupPlantId/pickupDate); PENDING→DITUGASKAN only; mirrors driver+vehicle onto linked siblings; **now also mirrors the fleet status itself** (releases the outgoing driver/vehicle if changed, engages the new pair per the shipment's status — fixed 2026-09-22; previously left driver/vehicle at ACTIVE/AVAILABLE after assign) |
...
- **Raw vs display status:** `rawStatus` (`AT_PLANT`) drives logic; `status` (`at_plant`, via `mapStatus`) drives badges. `mapStatus` folds legacy `FAILED` into `cancelled`, so the Dibatalkan tab queries `CANCELLED,FAILED`. The WhatsApp-notify-button render gate was comparing `status` against raw enum values (always false, button never rendered) — **fixed 2026-09-22**, now compares `rawStatus`.
- **Field vs tabbed layout:** `usesFieldLayout` swaps tabs→status dropdown, side panel→centered modal, and adds the Dalam Proses/Selesai toggle. "Selesai" = lifecycle split on server `TERMINAL_STATUSES` (DELIVERED/CANCELLED/FAILED), sorted by `COALESCE(completionDate, pickupDate, createdAt) DESC` — not a date filter.
- **SUPPORT is read-only on shipment status (fixed 2026-09-22).** Its old fallback DITUGASKAN flow sent TRANSIT but the server forward map only allows DITUGASKAN→AT_PLANT (always 403'd); its Ganti Driver picker collected a selection it never submitted; its PENDING branch swallowed errors. All three dead branches (in `handleConfirmStatus` and `renderModalContent`) were deleted along with the unused `newDriverVehicleId` state; `canUpdateStatus` now explicitly returns `false` for SUPPORT.
- **KEPALA_ARMADA PENDING branch is still dead** (package-8 territory, not yet cleaned): `canUpdateStatus` gives KA the modal only at STANDBY, so the `rawStatus === 'PENDING'` branch in `renderModalContent` (incl. free-text `assignPickupPlantId` sent into a FK column) is unreachable today.
- **Dead state (package-8 territory):** `serahTerimaUrl`, `handoverNotes`, `vehicleCondition`, `lkuNumber`, `pabrikNotes` are reset but never rendered/submitted; frontend `STATUS_SORT_RANK` is superseded by the server copy; exported `SERVICE_LABELS`/`STATUS_OPTIONS` have no importers.
